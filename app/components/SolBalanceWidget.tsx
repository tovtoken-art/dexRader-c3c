...
const Chip = ({ v, inline, label }: { v: number | null; inline?: boolean; label?: string }) => {
  ...
  if (inline) {
    const tone = v > 0 ? "text-emerald-600 dark:text-emerald-400" : v < 0 ? "text-rose-600 dark:text-rose-400" : "text-neutral-500 dark:text-neutral-400";
    return <span className={`text-[10px] sm:text-xs ${tone}`}>{label} {sign}{abs}</span>;
  }
  const tone =
    v > 0
  ...
  return <span className={`rounded-full px-2.5 py-1 text-xs font-medium tracking-tight ring-1 ring-inset ${tone}`}>{sign}{abs}</span>;
};
...
export default function SolBalanceWidget() {
  ...
  const lastTradeDelta = useRef<number | null>(null);
  ...
  const fetchBalance = async () => {
    ...
    try {
      ...
      const prev = bal;
      setBal(data.sol);
      setHist(h => (h[h.length-1] === data.sol ? h : [...h, data.sol].slice(-MAX_POINTS)));
      setTouchedAt(new Date());
      setErr(null);

      // 외부 입출금 등으로 인한 변화
      if (prev != null) {
        const diff = data.sol - prev;
        if (Math.abs(diff) >= DELTA_MIN) {
          const nearTrade = Date.now() - lastTradeAt.current < 3000;
          const sameAsTrade = lastTradeDelta.current != null && Math.abs(diff - lastTradeDelta.current) < DELTA_MIN;
          if (!(nearTrade && sameAsTrade)) setDeltas(d => [diff, ...(d || [])].slice(0, 2));
        }
      }
      return data.sol;
    } catch (e: any) {
  ...
  const seedFromTrades = async (w: string | null, baseBalance?: number | null) => {
    try {
      if (!w) return; // 지갑 확정 전엔 거래 시드하지 않음
      const { data } = await sb
        .from("trade_events")
  ...
  // 초기: 설정값 먼저 로드 → 잔고 → 거래 시드
  useEffect(() => {
    (async () => {
      try {
        const { data } = await sb.from("app_settings").select("value,updated_at").eq("key","balance_wallet").maybeSingle();
      ...
      // wallet이 아직 null이면 balance 응답의 address로 setWallet될 것. 시드는 wallet 변화를 기다림.
      if (wallet) await seedFromTrades(wallet, s);
    })();
  ...
  useEffect(() => {
    const ch = sb
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "app_settings", filter: "key=eq.balance_wallet" }, (p: any) => {
        const v = p?.new?.value as string | undefined;
        const upd = p?.new?.updated_at as string | undefined;
        if (typeof v === "string") {
          setWallet(v.length ? v : null);
          try { v.length ? localStorage.setItem("balance_wallet", v) : localStorage.removeItem("balance_wallet"); } catch {}
        }
        if (upd) setNextAt(new Date(upd).getTime() + COOLDOWN_MS);
      })
      .subscribe();
  ...
  useEffect(() => {
    (async () => {
      if (!wallet) return; // 지갑 확정 전엔 아무 것도 안 함
      if (!loading) { setHist([]); setBal(null); setDeltas([]); }
      const s = await fetchBalance();
      await seedFromTrades(wallet, s);
      lastTradeAt.current = 0;
      lastTradeDelta.current = null;
    })();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [wallet]);

  // 거래 실시간 구독: 항상 지갑 필터 붙임. 지갑 없으면 구독 자체를 만들지 않음
  useEffect(() => {
    if (!wallet) return; // ❗ 지갑 확정 전에는 글로벌 구독 금지
    const ch = sb
      ...
      .on("postgres_changes",
        ...
        async (p: any) => {
          ...
          if (d != null) {
            ...
            lastTradeDelta.current = d;
            setDeltas(prev => [d, ...(prev || [])].slice(0, 2));
          }
          await fetchBalance();
        }
      )
  ...
  const onEdit = async () => {
    ...
    if (val === "") {
      setWallet(null);
      try { const { error } = await sb.from("app_settings").upsert({ key: "balance_wallet", value: "" }); if (error) throw error; setNextAt(Date.now()+COOLDOWN_MS); }
      catch { await refreshCooldown(); }
    ...
    try {
      ...
      setWallet(addr);
      try { const { error } = await sb.from("app_settings").upsert({ key: "balance_wallet", value: addr }); if (error) throw error; setNextAt(Date.now()+COOLDOWN_MS); }
      catch { await refreshCooldown(); alert("30초 쿨다운 중이라 아직 변경할 수 없다"); }
      try { localStorage.setItem("balance_wallet", addr); } catch {}
    } catch { alert("유효한 Solana 지갑 주소가 아니다"); }
  };
      ...
      <div className="rounded-2xl bg-gradient-to-br from-emerald-400/40 via-fuchsia-400/40 to-cyan-400/40 p-[1px] shadow-[0_10px_30px_-10px_rgba(0,0,0,0.35)]">
        <div className="flex w-[320px] max-w-[90vw] flex-col gap-3 rounded-2xl border border-white/30 bg-white/80 p-4 backdrop-blur-xl dark:border-white/10 dark:bg-neutral-900/70">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="relative">
                <div className="grid h-8 w-8 place-items-center rounded-full bg-gradient-to-br from-neutral-900 to-neutral-700 text-white dark:from-emerald-500 dark:to-fuchsia-600">
                  <svg viewBox="0 0 24 24" className="h-4 w-4"><path fill="currentColor" d="M7 6h12l-2 2H5zM7 11h12l-2 2H5zM7 16h12l-2 2H5z" /></svg>
                </div>
            ...
            <div className="relative">
              {locked && <Ring p={ringP} />}
              <button type="button" onClick={onEdit} disabled={locked} aria-label="지갑 주소 설정"
          ...
          <div className="relative h-10 w-full">
            <svg viewBox="0 0 140 36" className="h-10 w-full overflow-visible">
              {points && <polyline points={points} className="fill-none stroke-current text-emerald-600 dark:text-emerald-400" strokeWidth="2" strokeLinejoin="round" strokeLinecap="round" />}
            </svg>
          ...
          {err && <div className="rounded-md border border-rose-500/30 bg-rose-500/10 p-2 text-xs text-rose-600 dark:text-rose-400">점검 후 다시 시도하세요</div>}
        </div>
      </div>
    </div>
  );
      ...
      <div className="pointer-events-auto border-t border-white/30 bg-white/85 backdrop-blur-xl shadow-[0_-12px_30px_-16px_rgba(0,0,0,0.35)] dark:border-white/10 dark:bg-neutral-900/70">
        ...
        <div className="mx-auto flex h-12 max-w-screen-2xl items-center justify-between gap-2 px-3">
          ...
          <div className="flex min-w-0 items-center justify-end gap-2">
            {!loading && !err && <Chip v={deltas[0] ?? null} inline label="" />} {/* 텍스트 */}
            <div className="relative">
            ...
            <button type="button" onClick={fetchBalance} disabled={spinning}
              aria-label="잔고 새로고침"
